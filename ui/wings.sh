...
ask_database_user() {
  echo -e -n "${COLOR_YELLOW}* Do you want to automatically configure a user for database hosts? (y/N): ${COLOR_NC}"
...
ask_database_external() {
  echo -e -n "${COLOR_YELLOW}* Do you want to configure MySQL to be accessed externally? (y/N): ${COLOR_NC}"
  read -r CONFIRM_DBEXTERNAL

  if [[ "$CONFIRM_DBEXTERNAL" =~ [Yy] ]]; then
    echo -e -n "${COLOR_YELLOW}* Enter the panel address (blank for any address): ${COLOR_NC}"
...
ask_database_firewall() {
  ...
  echo -e -n "${COLOR_YELLOW}* Would you like to allow incoming traffic to port 3306? (y/N): ${COLOR_NC}"
...
main() {
  ...
  echo "* "
  echo -e "* ${COLOR_YELLOW}The installer will install Docker, required dependencies for Wings${COLOR_NC}"
  echo -e "* ${COLOR_YELLOW}as well as Wings itself. But it's still required to create the node${COLOR_NC}"
  echo -e "* ${COLOR_YELLOW}on the panel and then place the configuration file on the node manually after${COLOR_NC}"
  echo -e "* ${COLOR_YELLOW}the installation has finished. Read more about this process on the${COLOR_NC}"
  ...
  if [ "$CONFIGURE_LETSENCRYPT" == true ]; then
    while [ -z "$FQDN" ]; do
      echo -e -n "${COLOR_YELLOW}* Set the FQDN to use for Let's Encrypt (node.example.com): ${COLOR_NC}"
      read -r FQDN
      ...
      [ "$ASK" == true ] && FQDN=""
      [ "$ASK" == true ] && echo -e -n "${COLOR_YELLOW}* Do you still want to automatically configure HTTPS using Let's Encrypt? (y/N): ${COLOR_NC}"
  ...
  if [ "$CONFIGURE_LETSENCRYPT" == true ]; then
    ...
    while ! valid_email "$EMAIL"; do
      echo -e -n "${COLOR_YELLOW}* Enter email address for Let's Encrypt: ${COLOR_NC}"
      read -r EMAIL
  ...
  echo -e -n "${COLOR_YELLOW}* Proceed with installation? (y/N): ${COLOR_NC}"
...
function goodbye {
  echo ""
  print_brake
  echo -e "* ${COLOR_GREEN}[+] Wings installation completed${COLOR_NC}"
  echo "*"
  echo -e "* ${COLOR_BOLD}To continue, you need to configure Wings to run with your panel${COLOR_NC}"
