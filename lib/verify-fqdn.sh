...
fail() {
  ...
  echo -e -n "${COLOR_YELLOW}* Proceed anyways (your install will be broken if you do not know what you are doing)? (y/N): ${COLOR_NC}"
  read -r override
...
confirm() {
  ...
  echo -e -n "${COLOR_YELLOW}* I agree that this HTTPS request is performed (y/N): ${COLOR_NC}"
  read -r confirm
