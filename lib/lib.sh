...
output() {
  echo -e "${COLOR_BLUE}[${APP_NAME}]${COLOR_NC} $1"
}
...
hyperlink() {
  echo -e "\e]8;;${1}\a${COLOR_CYAN}${1}${COLOR_NC}\e]8;;\a"
}
...
welcome() {
  ...
  clear
  print_brake
  echo -e "${COLOR_BOLD}${COLOR_CYAN}    $APP_NAME Installer @ $SCRIPT_RELEASE${COLOR_NC}"
  echo -e "${COLOR_CYAN}    Copyright (C) 2018 - 2025, $APP_NAME${COLOR_NC}"
  output "https://github.com/pterodactyl-installer/pterodactyl-installer"
  output ""
  output "Powered by ${COLOR_MAGENTA}KSGAMING${COLOR_NC}."
  output ""
  output "System: $OS $OS_VER ($ARCH)"
  
  if [ "$1" == "panel" ]; then
    output "Action: Installing Panel ${COLOR_GREEN}($PTERODACTYL_PANEL_VERSION)${COLOR_NC}"
  elif [ "$1" == "wings" ]; then
    output "Action: Installing Wings ${COLOR_GREEN}($PTERODACTYL_WINGS_VERSION)${COLOR_NC}"
  fi
  print_brake
  echo ""
}
...
required_input() {
  ...
  while [ -z "$result" ]; do
    echo -e -n "${COLOR_YELLOW}* ${2}${COLOR_NC}"
    read -r result
...
ask_firewall() {
  ...
  case "$OS" in
  ubuntu | debian)
    echo -e -n "${COLOR_YELLOW}* Do you want to automatically configure UFW (firewall)? (y/N): ${COLOR_NC}"
    read -r CONFIRM_UFW
    ...
    ;;
  rocky | almalinux)
    echo -e -n "${COLOR_YELLOW}* Do you want to automatically configure firewall-cmd (firewall)? (y/N): ${COLOR_NC}"
