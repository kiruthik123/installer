...
rm_database() {
  ...
  if [[ "$valid_db" == *"panel"* ]]; then
    echo -e -n "${COLOR_YELLOW}* Database called panel has been detected. Is it the pterodactyl database? (y/N): ${COLOR_NC}"
    read -r is_panel
    if [[ "$is_panel" =~ [Yy] ]]; then
      DATABASE=panel
    else
  ...
  while [ -z "$DATABASE" ] || [[ "$valid_db" != *"$DATABASE"* ]]; do
    echo -e -n "${COLOR_YELLOW}* Choose the panel database (to skip don't input anything): ${COLOR_NC}"
  ...
  if [[ "$valid_users" == *"pterodactyl"* ]]; then
    echo -e -n "${COLOR_YELLOW}* User called pterodactyl has been detected. Is it the pterodactyl user? (y/N): ${COLOR_NC}"
    read -r is_user
    if [[ "$is_user" =~ [Yy] ]]; then
      DB_USER=pterodactyl
    else
  ...
  while [ -z "$DB_USER" ] || [[ "$valid_users" != *"$DB_USER"* ]]; do
    echo -e -n "${COLOR_YELLOW}* Choose the panel user (to skip don't input anything): ${COLOR_NC}"
    read -r user_input
  ...
  success "Database and user removed (if selected)"
}
